...
export const removeCourse = async (req, res) => {
  try {
    ...
    // delete the course from cloudinary as well
    if (course.courseThumbnail) {
      await deleteMediaFromCloudinary(course?.courseThumbnail);
    }

    return res.status(200).json({
      message: "Course remove succcessfully",
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
...
export const SearchCourse = async(req,res) => {
  try {
    ...
    let courses = await Course.find(searchCriteria).populate({path:"creator", select:"name phtoUrl"}).sort(sortOptions);
    return res.status(200).json({
      success: true,
      courses: courses || []
    })
...
export const CreateLecture = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      lecture,
      message: "Lecture created succcessfully",
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
...
export const removeLecture = async (req, res) => {
  try {
    ...
    await Course.updateOne(
      { lectures: lectureId }, // find the course that coontains the lecture
